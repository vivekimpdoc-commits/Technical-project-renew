...
export default function CCTVSummarizerDetails({ project, onBack }) {
  ...
  const overviewSections = [
    { id: 'io-assistance', title: 'Time Compression', subtitle: '24 घंटे की फुटेज 5 मिनट में', icon: FastForward, color: 'var(--text-muted)' },
  ...
  const guideSteps = [
    ...
    { id: 6, title: 'चरण 6: UI/UX और डिप्लॉयमेंट', subtitle: 'Dashboard Integration', icon: Layout, color: 'var(--text-muted)' },
  ];
              ...
              <div style={{ background: '#0f172a', borderRadius: '1rem', padding: '2rem', color: 'white', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                  <div style={{ textAlign: 'center', flex: 1, minWidth: '150px' }}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>24h Footage (10GB)</div>
                  </div>
                  ...
                  <div style={{ textAlign: 'center', flex: 1, minWidth: '150px' }}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>YOLOv8 Detection</div>
                  </div>
                  ...
                  <div style={{ textAlign: 'center', flex: 1, minWidth: '150px' }}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>FFmpeg Processing</div>
                  </div>
                  ...
                  <div style={{ textAlign: 'center', flex: 1, minWidth: '150px' }}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>5m Summary Video</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'architecture' && (
            <div className="animate-fade-in">
              ...
              <div style={{ background: 'var(--icon-bg)', padding: '2rem', borderRadius: '1rem', border: '1px solid var(--card-border)', marginBottom: '2rem' }}>
                <h3 style={{ fontSize: '1.2rem', color: 'var(--text-main)', marginBottom: '1.5rem', fontWeight: 'bold' }}>1. Video Ingestion Layer (फीड ग्रहण)</h3>
                <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: '1.6' }}>RTSP स्ट्रीम या अपलोड की गई MP4 फाइल्स को सिस्टम में लिया जाता है। OpenCV का उपयोग करके वीडियो को फ्रेम्स में तोड़ा जाता है।</p>

                <h3 style={{ fontSize: '1.2rem', color: 'var(--text-main)', marginBottom: '1.5rem', marginTop: '2rem', fontWeight: 'bold' }}>2. AI Inference Engine (AI प्रोसेसिंग)</h3>
                <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: '1.6' }}>YOLOv8 या Faster R-CNN मॉडल हर N-वें फ्रेम (जैसे हर 5वें फ्रेम) का विश्लेषण करता है ताकि प्रोसेसिंग तेज हो। यह इंसान या गाड़ी का bounding box डिटेक्ट करता है।</p>

                <h3 style={{ fontSize: '1.2rem', color: 'var(--text-main)', marginBottom: '1.5rem', marginTop: '2rem', fontWeight: 'bold' }}>3. Logic & Stitching Layer (क्लिप्स जोड़ना)</h3>
                <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: '1.6' }}>जिन फ्रेम्स में ऑब्जेक्ट डिटेक्ट होता है, उन टाइमस्टैम्प्स (जैसे 12:04:10 से 12:04:30) को एक लिस्ट में सेव किया जाता है। फिर FFmpeg का उपयोग करके इन सभी टाइम-सेगमेंट्स को मुख्य वीडियो से काटकर एक नई वीडियो फ़ाइल में जोड़ (Stitch) दिया जाता है।</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'tech-stack' && (
            <div className="animate-fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
                ...
                <div style={{ border: '1px solid var(--card-border)', borderRadius: '1rem', padding: '1.5rem', background: 'var(--card-bg)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem', color: '#10b981' }}><Server size={24} /> <h3 style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--text-main)', margin: 0 }}>Backend & Processing</h3></div>
                  <ul style={{ paddingLeft: '1.5rem', color: 'var(--text-muted)', lineHeight: '1.8' }}>
            ...
            <div className="animate-fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                <div style={{ background: 'var(--icon-bg)', padding: '2rem', borderRadius: '1rem', border: '1px solid var(--card-border)' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--text-main)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Database size={20} color={project?.color || '#3b82f6'} /> Datasets</h3>
                  <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: 0, margin: 0, listStyle: 'none' }}>
                    <li style={{ background: 'var(--card-bg)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}>
                      <strong>VIRAT Video Dataset:</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Surveillance video dataset for event recognition.</span>
                    </li>
                    <li style={{ background: 'var(--card-bg)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}>
                      <strong>UCF-Crime:</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Real-world CCTV videos covering anomalies like robbery, stealing.</span>
                    </li>
                  </ul>
                </div>
                <div style={{ background: 'var(--icon-bg)', padding: '2rem', borderRadius: '1rem', border: '1px solid var(--card-border)' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--text-main)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><BookOpen size={20} color={project?.color || '#3b82f6'} /> Libraries</h3>
                  <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: 0, margin: 0, listStyle: 'none' }}>
                    <li style={{ background: 'var(--card-bg)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}>
                      <strong>Ultralytics YOLOv8:</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Pre-trained object detection models.</span>
                    </li>
                    <li style={{ background: 'var(--card-bg)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}>
                      <strong>FFmpeg-python:</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Python wrapper for complex video stitching.</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div style={{ background: 'var(--icon-bg)', padding: '2rem', borderRadius: '1rem', border: '1px solid var(--card-border)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '1.5rem', borderBottom: '1px solid var(--card-border)' }}>
                    <div>
                      <h4 style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>Sensitivity Threshold</h4>
                      <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>मोशन डिटेक्शन की संवेदनशीलता सेट करें।</p>
                    </div>
                    <select style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}><option>High</option><option selected>Medium</option><option>Low</option></select>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <h4 style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>Buffer Time</h4>
                      <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>इवेंट क्लिप के आगे-पीछे का एक्स्ट्रा समय।</p>
                    </div>
                    <select style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}><option>2 Seconds</option><option selected>5 Seconds</option><option>10 Seconds</option></select>
                  </div>
                </div>
              </div>
            </div>
