...
import { 
  ArrowLeft, LayoutDashboard, FileText, Code, ListOrdered, BookOpen, CheckSquare, Settings, Target, Shield, Database, Layout, CheckCircle, Network, Server, Briefcase, Activity, Monitor, BrainCircuit, Search, Zap, AlertTriangle, MessageSquare, Video, MicOff, Type
} from 'lucide-react';
import './ProjectDetailsView.css';

export default function LipReadingAIDetails({ project, onBack }) {
  ...
  const guideSteps = [
    { id: 1, title: 'चरण 1: वीडियो प्री-प्रोसेसिंग', subtitle: 'Frame Extraction', icon: Video, color: '#3b82f6' },
    { id: 2, title: 'चरण 2: लिप ट्रैकिंग (Mouth ROI)', subtitle: 'Facial Landmark', icon: Search, color: '#10b981' },
    { id: 3, title: 'चरण 3: 3D-CNN मॉडल ट्रेनिंग', subtitle: 'Lip2Wav / LRW', icon: BrainCircuit, color: '#f59e0b' },
    { id: 4, title: 'चरण 4: डिकोडर (Seq2Seq)', subtitle: 'Text Generation', icon: Type, color: '#8b5cf6' },
    { id: 5, title: 'चरण 5: UI और सबटाइटल ओवरले', subtitle: 'Dashboard', icon: Monitor, color: '#ec4899' },
    { id: 6, title: 'चरण 6: एक्यूरेसी टेस्टिंग', subtitle: 'Validation', icon: Shield, color: '#64748b' },
  ];

  const evaluationList = [
    { id: 1, title: '1. टेक्स्ट सटीकता (WER)', subtitle: 'Word Error Rate', icon: CheckSquare, color: '#3b82f6' },
    { id: 2, title: '2. वीडियो क्वालिटी टॉलरेंस', subtitle: 'Low-Res Handling', icon: Activity, color: '#10b981' },
    { id: 3, title: '3. रीजनल एक्सेंट और भाषा', subtitle: 'Language Model', icon: Target, color: '#f59e0b' },
    { id: 4, title: '4. अदालती मान्यता', subtitle: 'Legal Standing', icon: Shield, color: '#8b5cf6' },
  ];
  ...
  if (showFullscreenFlowchart) {
    return (
      <div className="fullscreen-flow-view" style={{ padding: '2rem', background: '#f8fafc', minHeight: '100vh', width: '100%' }}>
        ...
        <h3 style={{ textAlign: 'center', marginBottom: '3rem', fontSize: '1.75rem', color: '#0f172a' }}>Lip-Reading AI Architecture</h3>
        <div className="flowchart-container" style={{ margin: '0 auto', maxWidth: '800px', background: 'white', padding: '2rem', borderRadius: '1rem', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}>
          <div className="flow-step"><div className="flow-node user-node">Silent CCTV Video Input</div></div>
          <div className="flow-arrow">↓</div>
          <div className="flow-step"><div className="flow-node">Face & Mouth Region of Interest (ROI) Cropping</div></div>
          <div className="flow-arrow">↓</div>
          <div className="flow-step"><div className="flow-node">Spatio-Temporal Visual Feature Extraction (3D-CNN)</div></div>
          <div className="flow-arrow">↓</div>
          <div className="flow-step"><div className="flow-node">Sequence-to-Sequence Text Decoder (Transformer/LSTM)</div></div>
          <div className="flow-arrow">↓</div>
          <div className="flow-step"><div className="flow-node output-node">Video Playback with AI Subtitles & Confidence Score</div></div>
        </div>
      </div>
    );
  }
  ...
  if (showFullscreenWorkflow) {
    return (
      <div className="fullscreen-flow-view" style={{ padding: '2rem', background: '#f8fafc', minHeight: '100vh', width: '100%' }}>
        <button onClick={() => setShowFullscreenWorkflow(false)} className="back-button" style={{ marginBottom: '2rem', display: 'inline-flex', padding: '0.75rem 1.5rem', background: 'white', borderRadius: '0.5rem', color: '#0f172a', border: '1px solid #cbd5e1', fontWeight: 'bold', cursor: 'pointer' }}>
          <ArrowLeft size={18} /> Back to Architecture Menu
        </button>
        ...
        <div className="workflow-container" style={{ maxWidth: '900px', margin: '0 auto' }}>
          <div className="workflow-step-card" style={{ borderLeftColor: '#3b82f6' }}>
            <h4 className="workflow-step-title">1. Video Input</h4>
            <div className="workflow-point">बिना आवाज वाला CCTV या मोबाइल फुटेज सॉफ्टवेयर में डाला जाता है।</div>
          </div>
          <div className="workflow-step-card" style={{ borderLeftColor: '#10b981' }}>
            <h4 className="workflow-step-title">2. Lip Tracking</h4>
            <div className="workflow-point">AI वीडियो में मौजूद लोगों के चेहरे पहचान कर केवल उनके होठों (Lips) के मूवमेंट पर फोकस करता है।</div>
          </div>
          <div className="workflow-step-card" style={{ borderLeftColor: '#f59e0b' }}>
            <h4 className="workflow-step-title">3. Visual Word Recognition</h4>
            <div className="workflow-point">होठों के आकार और मूवमेंट की तुलना AI मॉडल अपनी डिक्शनरी से करता है।</div>
          </div>
          <div className="workflow-step-card" style={{ borderLeftColor: '#8b5cf6' }}>
            <h4 className="workflow-step-title">4. Output Display</h4>
            <div className="workflow-point">स्क्रीन पर वीडियो के साथ-साथ अनुमानित बातचीत (Subtitles) और AI का Confidence Percentage दिखाई देता है।</div>
          </div>
        </div>
      </div>
    );
  ...
  if (selectedOverviewSection !== null) {
    ...
    return (
      <div className="fullscreen-flow-view" style={{ padding: '2rem', background: '#f8fafc', minHeight: '100vh', width: '100%' }}>
        <button onClick={() => setSelectedOverviewSection(null)} className="back-button" style={{ marginBottom: '2rem', display: 'inline-flex', padding: '0.75rem 1.5rem', background: 'white', borderRadius: '0.5rem', color: '#0f172a', border: '1px solid #cbd5e1', fontWeight: 'bold', cursor: 'pointer' }}>
                ...
                case 'io-assistance': return (
                  <div style={{ background: '#ffffff', padding: '2rem', borderRadius: '1rem', border: '1px solid #e2e8f0' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1rem' }}>
                      <div className="workflow-point">• मूक (Silent) CCTV वीडियो में अपराधी आपस में क्या बात कर रहे हैं, इसका अनुमान लगाना।</div>
                      <div className="workflow-point">• दूर से रिकॉर्ड किये गए वीडियो (जिसमे आवाज न हो) से महत्वपूर्ण लीड्स प्राप्त करना।</div>
                      <div className="workflow-point">• पूछताछ के दौरान आरोपी द्वारा धीरे से फुसफुसाए गए शब्दों को पकड़ना।</div>
                    </div>
                  </div>
                );
                case 'objectives': return (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem' }}>
                    <div className="tech-category-card" style={{ borderTop: '4px solid #3b82f6' }}>
                      <h5 style={{ margin: '0 0 1rem 0' }}><Video size={20}/> Visual Speech Recognition</h5>
                      <span className="workflow-point">वीडियो फ्रेम्स को टेक्स्ट (Speech-to-Text without Audio) में बदलना।</span>
                    </div>
                    <div className="tech-category-card" style={{ borderTop: '4px solid #10b981' }}>
                      <h5 style={{ margin: '0 0 1rem 0' }}><Type size={20}/> Probabilistic Subtitles</h5>
                      <span className="workflow-point">विभिन्न अनुमानों और सटीकता स्कोर (Confidence Score) के साथ टेक्स्ट देना।</span>
                    </div>
                  </div>
                );
                case 'features': return (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
                    <div className="tech-category-card"><MicOff size={22} color="#475569"/> <h5>Silent AI Decoding</h5><p>बिना ऑडियो के काम</p></div>
                    <div className="tech-category-card"><Type size={22} color="#475569"/> <h5>Subtitle Overlay</h5><p>वीडियो पर टेक्स्ट</p></div>
                    <div className="tech-category-card"><Search size={22} color="#475569"/> <h5>Mouth Tracking</h5><p>चेहरे के मूवमेंट का विश्लेषण</p></div>
                    <div className="tech-category-card"><AlertTriangle size={22} color="#475569"/> <h5>Confidence Score</h5><p>नतीजों की संभावना</p></div>
                  </div>
                );
                default: return null;
              }
  ...
  return (
    <div className="project-details-container">
      <div className="details-header-bar">
        <button onClick={onBack} className="back-button"><ArrowLeft size={18} /> Back to Dashboard</button>
        <div className="details-header-content">
          <div className="details-title-wrapper">
            <h2 className="details-title">{project?.name || 'AI "लिप-रीडिंग" फॉर साइलेंट CCTV'}</h2>
        ...
        <div className="details-content-area">
          {activeTab === 'overview' && (
            <div className="details-card" style={{ border: 'none', padding: 0 }}>
              <div style={{ background: 'linear-gradient(135deg, #0f766e 0%, #115e59 100%)', borderRadius: '1rem', padding: '2.5rem', color: 'white', marginBottom: '2.5rem', position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: '-10%', right: '-5%', opacity: 0.1 }}><MessageSquare size={200} /></div>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '1.75rem', margin: '0 0 1rem 0' }}>
                  <MessageSquare size={28} color="#5eead4" /> Project Overview
                </h3>
                <p style={{ color: '#ccfbf1', fontSize: '1.1rem', lineHeight: '1.7', maxWidth: '90%' }}>
                  <strong>"Lip-Reading AI"</strong> एक कंप्यूटर विजन मॉडल है जो मूक (Silent) CCTV वीडियो में अपराधियों के होठों (Lips) के मूवमेंट को पढ़कर उनकी बातचीत का टेक्स्ट (Subtitles) जनरेट करता है।
                </p>
              </div>
