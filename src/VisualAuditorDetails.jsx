...
export default function VisualAuditorDetails({ project, onBack }) {
  ...
  const overviewSections = [
    { id: 'io-assistance', title: 'Police Uniform Audit', subtitle: 'ड्यूटी पर सही वर्दी की जांच', icon: CheckSquare, color: 'var(--text-muted)' },
  ...
  const guideSteps = [
    ...
    { id: 6, title: 'चरण 6: Officer Dashboard', subtitle: 'React UI for Reports', icon: Layout, color: 'var(--text-muted)' },
  ];
        ...
        <div style={{ background: 'var(--card-bg)', borderRadius: '1rem', border: '1px solid var(--card-border)', minHeight: '600px', padding: '2.5rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(16, 185, 129, 0.1)', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Uniform, Cap detected</div>
                        </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(239, 68, 68, 0.1)', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Violation: Cap Missing</div>
                        </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(239, 68, 68, 0.1)', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Violation: Pillion no helmet</div>
                        </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(16, 185, 129, 0.1)', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Compliant: Rider & Pillion safe</div>
                        </div>
                        <CheckCircle color="#34d399" size={18} />
                      </div>
                    </div>
          ...
          {activeTab === 'architecture' && (
             ...
             <div style={{ background: 'var(--icon-bg)', padding: '2rem', borderRadius: '1rem', border: '1px solid var(--card-border)', marginBottom: '2rem' }}>
               <h3 style={{ fontSize: '1.2rem', color: 'var(--text-main)', marginBottom: '1.5rem', fontWeight: 'bold' }}>1. Object Detection (इंसान और वाहन खोजना)</h3>
               <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: '1.6' }}>स्ट्रीमिंग वीडियो में से YOLOv8 मॉडल का उपयोग करके 'Person' और 'Motorcycle' को डिटेक्ट किया जाता है। यदि डिटेक्ट किया गया ऑब्जेक्ट मोटरसाइकिल है, तो मॉडल देखता है कि उस पर कितने लोग बैठे हैं (Rider and Pillion)।</p>
               
               <h3 style={{ fontSize: '1.2rem', color: 'var(--text-main)', marginBottom: '1.5rem', marginTop: '2rem', fontWeight: 'bold' }}>2. Head Detection & Classification (सिर की जांच)</h3>
               <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: '1.6' }}>अगले स्टेप में, डिटेक्ट किए गए व्यक्ति के ऊपरी हिस्से (Upper Body/Head) को क्रॉप किया जाता है। एक सेकेंडरी CNN मॉडल यह चेक करता है कि उस व्यक्ति के सिर पर हेलमेट/टोपी है या नहीं। पगड़ी (Turban) को पहचानने के लिए विशेष ट्रेनिंग दी जाती है ताकि सिखों का चालान न कटे।</p>
               
               <h3 style={{ fontSize: '1.2rem', color: 'var(--text-main)', marginBottom: '1.5rem', marginTop: '2rem', fontWeight: 'bold' }}>3. Evidence Capture & Integration (सबूत और चालान)</h3>
               <p style={{ color: 'var(--text-muted)', marginBottom: '1rem', lineHeight: '1.6' }}>जब किसी उल्लंघन की पुष्टि हो जाती है (Probability &gt; 85%), तो सिस्टम उस फ्रेम की फोटो खींचकर डेटाबेस में सेव कर देता है। नंबर प्लेट (ANPR) को पढ़कर सीधे ट्रैफिक पुलिस के ई-चालान पोर्टल को API रिक्वेस्ट भेज दी जाती है।</p>
             </div>
           </div>
          )}
          ...
          {activeTab === 'tech-stack' && (
            <div className="animate-fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
                ...
                <div style={{ border: '1px solid var(--card-border)', borderRadius: '1rem', padding: '1.5rem', background: 'var(--card-bg)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem', color: '#ec4899' }}><BrainCircuit size={24} /> <h3 style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--text-main)', margin: 0 }}>AI / Vision Models</h3></div>
                  <ul style={{ paddingLeft: '1.5rem', color: 'var(--text-muted)', lineHeight: '1.8' }}>
            ...
            <div className="animate-fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                <div style={{ background: 'var(--icon-bg)', padding: '2rem', borderRadius: '1rem', border: '1px solid var(--card-border)' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--text-main)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Database size={20} color={project?.color || '#ef4444'} /> Datasets</h3>
                  <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: 0, margin: 0, listStyle: 'none' }}>
                    <li style={{ background: 'var(--card-bg)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}>
                      <strong>Helmet Detection Dataset:</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Roboflow dataset containing thousands of images of riders with and without helmets.</span>
                    </li>
                    <li style={{ background: 'var(--card-bg)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}>
                      <strong>Custom Police Uniforms:</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>A proprietary dataset to be collected from CCTV for local police Khaki identification.</span>
                    </li>
                  </ul>
                </div>
                <div style={{ background: 'var(--icon-bg)', padding: '2rem', borderRadius: '1rem', border: '1px solid var(--card-border)' }}>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--text-main)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><BookOpen size={20} color={project?.color || '#ef4444'} /> Libraries</h3>
                  <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: 0, margin: 0, listStyle: 'none' }}>
                    <li style={{ background: 'var(--card-bg)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}>
                      <strong>YOLOv8:</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>For fast bounding box detection.</span>
                    </li>
                    <li style={{ background: 'var(--card-bg)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}>
                      <strong>Celery:</strong> <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>For processing violations in the background.</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div style={{ background: 'var(--icon-bg)', padding: '2rem', borderRadius: '1rem', border: '1px solid var(--card-border)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '1.5rem', borderBottom: '1px solid var(--card-border)' }}>
                    <div>
                      <h4 style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>Auto-Challan Generation</h4>
                      <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>क्या बिना मैन्युअल चेक के सीधा चालान भेजना है?</p>
                    </div>
                    <select style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem', border: '1px solid var(--card-border)' }}><option>Yes (Fully Automatic)</option><option selected>No (Require Manual Approval)</option></select>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <h4 style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>Turban Exception</h4>
                      <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>पगड़ी (Turban) को हेलमेट नियम से छूट दें।</p>
                    </div>
